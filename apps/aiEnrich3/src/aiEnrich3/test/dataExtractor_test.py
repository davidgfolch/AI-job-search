...
def mock_dependencies():
    with (
        patch("aiEnrich3.dataExtractor.get_job_enabled", return_value=True),
        patch("aiEnrich3.dataExtractor.MysqlUtil") as mock_mysql_util,
...
@pytest.mark.parametrize(
    "total_pending, error_id, initial_pipeline, expected_processed",
    [
        ...
        (
            ...
            None,
            10,
        ),  # Scenario 2: Pending, no error, no pipeline (creates pipeline)
        (
            0,
            123,
            None,
            5,
        ),  # Scenario 3: No pending, error, no pipeline (creates pipeline)
        (2, 456, "existing_pipe", 15),  # Scenario 4: Pending, error, existing pipeline
    ],
)
def test_dataExtractor(
    total_pending, error_id, initial_pipeline, expected_processed, mock_dependencies
):
    ...
    pipeline_arg = (
        deps["pipeline_instance"] if initial_pipeline == "existing_pipe" else None
    )
