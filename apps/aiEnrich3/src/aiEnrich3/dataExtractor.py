...
from aiEnrich3.config import get_batch_size
...
def dataExtractor(pipeline: Optional[ExtractionPipeline] = None) -> Tuple[int, Optional[ExtractionPipeline]]:
    with MysqlUtil() as mysql:
        ...
        if pipeline is None:
            print(yellow(f"Loading Local CPU Pipeline (GLiNER & mDeBERTa) as there are {total_pending} jobs to process..."))
