...
class TestIndeedScrapper:
    ...
    def test_run_preload_page(self, mock_selenium):
        with patch('scrapper.indeed.JOBS_SEARCH', 'python developer'), \
            ...
            mock_nav_instance.load_page.assert_called()
    ...
    def test_search_jobs_pagination(self):
        ...
        with patch('scrapper.indeed.IndeedNavigator') as mock_nav_class, \
             patch('scrapper.indeed.IndeedService') as mock_service_class, \
            ...
            # 15 jobs per page * 2 pages = 30 calls
    ...
    def test_process_row_insert(self):
        ...
        mock_navigator.get_job_data.return_value = ("Job Title", "Company Name", "Location", "<html>...</html>")
        mock_navigator.check_easy_apply.return_value = False
        
        # Setup service return values
        mock_service.process_job.return_value = True
        
        with patch('scrapper.indeed.navigator', mock_navigator), \
            ...
            mock_service.process_job.assert_called_once_with("Job Title", "Company Name", "Location", "http://job.url", "<html>...</html>", False)
    ...
    def test_service_process_job_insert(self, mock_mysql):
        ...
        with patch('scrapper.services.IndeedService.validate', return_value=True), \
             patch('scrapper.services.IndeedService.htmlToMarkdown', return_value="Description"), \
             patch('scrapper.services.IndeedService.mergeDuplicatedJobs'):
             
             mock_mysql.insert.return_value = 1
             
             result = service.process_job("Title", "Company", "Location", "http://url?jk=123", "<html/>", False)
             
             assert result is True
             mock_mysql.insert.assert_called_once()
             assert "123" in mock_mysql.insert.call_args[0][0]
             assert "Title" in mock_mysql.insert.call_args[0][0]

    def test_service_process_job_validation_fail(self, mock_mysql):
        service = IndeedService(mock_mysql, MagicMock())
        service.set_debug(False)

        with patch('scrapper.services.IndeedService.validate', return_value=False), \
             patch('scrapper.services.IndeedService.htmlToMarkdown', return_value="Description"):
