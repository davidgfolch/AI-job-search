...
@patch("aiEnrichNew.skillEnricher.get_skill_enabled", return_value=True)
def test_skillEnricher_calls_service(
    mock_enabled, mock_enrich, mock_pipe_fac, mock_mysql
):
    mysql = MagicMock()
