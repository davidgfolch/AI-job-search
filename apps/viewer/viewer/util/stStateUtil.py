...
from commonlib.util import createFolder, listFiles, toBool
...
def getSessionFileName(name: str) -> str:
    if name.endswith('.json'):
        return f'viewer/.stSessionState/{name}'
    return f'viewer/.stSessionState/{name}.json'


def listSessionFiles(removeExtension=True) -> str:
    path = Path('viewer/.stSessionState')
