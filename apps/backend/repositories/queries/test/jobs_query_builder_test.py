...
def test_build_jobs_where_clause_search(
    search, expected_clause_part, expected_param_count
):
...
@pytest.mark.parametrize(
    ...
    ids=["duplicated_true", "duplicated_false", "flagged_true"],
)
...
@pytest.mark.parametrize(
    "modality, expected_clauses, expected_param_count",
    [
        (None, [], 0),
        ([], [], 0),
        (["REMOTE"], ["modality IN (%s)"], 1),
        (["REMOTE", "HYBRID"], ["modality IN (%s, %s)"], 2),
        (["NULL"], ["modality IS NULL"], 0),
        (["REMOTE", "NULL"], ["(modality IN (%s) OR modality IS NULL)"], 1),
    ],
    ids=["none", "empty", "single_remote", "multiple", "null_only", "remote_and_null"],
)
def test_build_jobs_where_clause_modality(
    modality, expected_clauses, expected_param_count
):
    where, params = build_jobs_where_clause(
        search=None,
