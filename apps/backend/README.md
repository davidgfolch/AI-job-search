...
- **Settings API**: Read and write `.env` / `.env.secrets` variables and `scrapper_state.json` from the UI.
...
- `services/settings_service.py`: Business logic for reading/writing `.env` / `.env.secrets` and `scrapper_state.json`.
