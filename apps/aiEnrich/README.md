...
**Repetition penalty**: the Ollama backend always sends `repeat_penalty=1.3` (higher than the server default `1.1`) because Qwen models can degenerate into repetition loops that consume the entire `num_predict` budget and truncate the JSON output mid-string. Override with `AI_ENRICH_REPEAT_PENALTY` (e.g. `1.5`) to suppress loops harder. The model's context window is not the cause of that failure mode.
