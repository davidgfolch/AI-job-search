...
dependant_scripts() {
    terminator --new-tab --title="Ollama logs" --command="bash -c ' clear && sudo journalctl  -u ollama -f | perl ~/scripts/colorTail.pl \"gpu|cuda\"; exec bash'"
    sleep 3
...
terminator --new-tab --title="run_1_Mysql" --command="bash -c './scripts/run_1_Mysql.sh; exec bash'" && \
    wait_for_mysql && \
    dependant_scripts
