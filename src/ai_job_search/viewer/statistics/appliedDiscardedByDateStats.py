...
def run():
    with MysqlUtil() as mysql:
        ...
        df_melted = df.melt(id_vars='dateCreated',
                            value_vars=['applied', 'discarded', 'interview',
                                        'discarded_cumulative', 'interview_cumulative'],
                            var_name='category',
                            value_name='count')
        ...
        chart = alt.Chart(df_melted).mark_line().encode(
            x=alt.X('dateCreated:T', title='Date created',
                    axis=alt.Axis(format='%Y-%m-%d')),
            y=alt.Y('count:Q', title='Number of jobs'),
            color=alt.Color('category:N', scale=color_scale,
                            legend=alt.Legend(title="Legend")),
            strokeDash=alt.StrokeDash(
                'category:N', scale=line_dash_scale, legend=None),  # Estilo de línea
