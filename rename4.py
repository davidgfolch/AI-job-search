...
def get_new_var_name(app_name, var_name):
    ...
    res = prefix + suffix
    # per user request: Change AI_CVMATCHER_CV_MATCH* by AI_CVMATCHER*
...
for root, dirs, files in os.walk(apps_dir):
    # skip hidden and node_modules entirely
    dirs[:] = [d for d in dirs if not d.startswith('.') and d not in ('node_modules', '__pycache__', 'dist', 'build', '.pytest_cache')]
