...
from ai_job_search.scrapper_main import SCRAPPERS, runAllScrappers, runSpecifiedScrappers
...
@pytest.fixture
def mock_selenium():
    # Patch the module-level seleniumUtil object used in scrapper_main so
    # calls like seleniumUtil.tab(...) don't fail during tests.
    with patch('scrapper_main.seleniumUtil', MagicMock()):
        yield


def test_runAllScrappers(mock_scrappers, mock_selenium):
    with patch('scrapper_main.consoleTimer', return_value=None), \
            patch('scrapper_main.getDatetimeNow', return_value=0), \
            patch('scrapper_main.getSeconds', return_value=0):
