...
noise, all off by default: "Show external/library nodes" (imported modules,
symbols, and type annotations like `Any`, shown in a muted color), "Show file
nodes", and "Show private methods" (`_foo()` / `._foo()` / dunders like
`.__init__()`). The matrix view is generated by `scripts/graphify-html-grouped.py`
(Step 5 of the pipeline), which fills the HTML/CSS/JS template at
`scripts/templates/graphify-html.tpl`; re-run `scripts/graphify.sh` /
`scripts/graphify.bat` to regenerate it.
