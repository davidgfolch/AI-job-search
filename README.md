...
The interactive `graph.html` groups communities by app/module: the sidebar
legend shows one section per module (with a per-module toggle), and node colors
are assigned per module (base hue per module, shade per community) so the graph
is visually organized by app while keeping every edge, including cross-module
edges, intact. Python/external library nodes (imported modules, symbols, and
type annotations like `Any`) are hidden by default — tick "Show external/library
nodes" in the sidebar to reveal them in a muted color. The grouped view is
generated by `scripts/graphify-html-grouped.py` (Step 5 of the pipeline), which
fills the HTML/CSS/JS template at `scripts/templates/graphify-html.tpl`; re-run
`scripts/graphify.sh` / `scripts/graphify.bat` to regenerate it.
